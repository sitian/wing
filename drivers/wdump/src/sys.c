							...
							unsigned long flags, unsigned long pgoff)
{
	unsigned long (*syscall)(struct file *, unsigned long, 
							unsigned long, unsigned long, 
							unsigned long, unsigned long) = (void *)DO_MMAP_PGOFF;
	return syscall(file, addr, len, prot, flags, pgoff);
}
...
{
	void (*syscall)(struct fs_struct *, struct path *) = (void *)SET_FS_ROOT;
	syscall(fs, path);
}
...
{
	void (*syscall)(struct fs_struct *, struct path *) = (void *)SET_FS_PWD;
	syscall(fs, path);
}
...
{
	void (*syscall)(struct vm_area_struct *, unsigned long) = (void *)FLUSH_TLB_PAGE;
	syscall(vma, addr);
}
